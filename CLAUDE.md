...
**1つの機能（1セクション）の実装が終わったら、必ず作業を止めて動作確認を依頼すること。**

- 承認を得るまで、次の機能に着手してはいけない
- 複数の機能をまとめて実装して、最後に一括で確認を求めるのは禁止
- 「ついでにこれも直しておきました」も禁止。スコープ外の変更は提案に留める

### 動作確認を依頼するときのフォーマット

実装完了時は、必ず以下の形式で報告すること。

```
## 実装完了：<機能名>

### 変更したファイル
- path/to/file1.php（新規 / 修正）
- path/to/file2.tsx（新規 / 修正）

### 実装方針
（なぜこの実装にしたか、2〜3行）

### テスト結果
（php artisan test の実行結果。何件通ったか）
...
### 確認事項・懸念点
（あれば。設計上の判断に迷った点、後で見直したい点）

---
確認をお願いします。問題なければ次の機能に進みます。
```

### その他のルール

- 実装前に、変更するファイルと方針を提示して確認を取る
- テストを先に書き、その後実装する
- スキーマ変更が必要になった場合は、実装前に必ず確認を取る（`docs/database/er-diagram.md` の更新も必要）
- 判断に迷ったら、勝手に決めずに質問する